...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .top-actions { display: flex; justify-content: flex-end; gap: 10px; margin-bottom: 12px; flex-wrap: wrap; }
    ...
    .secondary { background: #d4d4d8; padding: 12px 14px; border-radius: 999px; }
    .reject { background: #fb7185; }
    .approve { background: #34d399; }
    video { width: 100%; border-radius: 20px; background: #000; border: 1px solid rgba(255,255,255,0.12); }
...
<body>
  <main>
    ...
    <div class="top-actions"><button class="secondary" id="review-videos">Review rendered videos</button><button class="render" id="render-approved">Render next approved</button></div>
  ...
  <script>
    ...
    async function renderNextApproved() {
      ...
      const res = await fetch('/reels/' + encodeURIComponent(reel.id) + '/render', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ mode: 'remotion' }),
      });
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
    ...
    async function reviewRenderedVideos() {
      const payload = await fetch('/reels?status=video_ready').then(res => res.json());
      const reel = (payload.data || [])[0];
      count.textContent = String((payload.data || []).length);
      if (!reel) {
        deck.innerHTML = '<div class="empty">No rendered videos waiting for final approval.</div>';
        return;
      }
      deck.innerHTML = '<article class="card">' +
        '<div class="project"><strong>' + escapeHtml(reel.projectSlug) + '</strong><span class="channel">rendered</span></div>' +
        '<h2>' + escapeHtml(reel.title) + '</h2>' +
        (reel.assetUrl ? '<video controls playsinline src="' + escapeHtml(reel.assetUrl) + '"></video>' : '<div class="brief">No video URL on this render.</div>') +
        '<p class="hook">' + escapeHtml(reel.hook) + '</p>' +
        '<div class="actions"><button class="reject" id="reject-video">Reject video</button><button class="approve" id="accept-video">Ready to post</button></div>' +
      '</article>';
      document.querySelector('#reject-video').onclick = () => decideVideo(reel.id, 'reject');
      document.querySelector('#accept-video').onclick = () => decideVideo(reel.id, 'approve');
    }

    async function decideVideo(id, decision) {
      const res = await fetch('/reels/' + encodeURIComponent(id) + '/video-decision', {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ decision }),
      });
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        deck.innerHTML = '<div class="error">' + escapeHtml(payload.error || 'Video decision failed') + '</div>';
        return;
      }
      deck.innerHTML = '<div class="empty">' + (decision === 'approve' ? 'Video accepted. Ready for posting.' : 'Video rejected.') + '</div>';
    }
    ...
    document.querySelector('#review-videos').onclick = reviewRenderedVideos;
    load().catch(error => {

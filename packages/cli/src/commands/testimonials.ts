...
export async function testimonialsListCommand(
  options: TestimonialsListOptions = {}
): Promise<void> {
  ...
  try {
    ...
    if (items.length === 0) {
      if (!options.quiet) log.info('No testimonials yet.');
      return;
    }
    printOutput(items, {
      ...
      raw: options.raw,
      defaultColumns: ['id', 'author_name', 'content', 'status', 'created_at'],
    });
  } catch (err) {
    spinner?.stop();
...
export async function testimonialsUpdateCommand(
  id: string,
  options: TestimonialsUpdateOptions = {}
): Promise<void> {
  const status = options.status;
  if (!status) {
    log.error('--status is required (approved, rejected, pending).');
    process.exitCode = 1;
    return;
  }

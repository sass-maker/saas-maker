...
export async function changelogListCommand(options: ChangelogListOptions = {}): Promise<void> {
  ...
  try {
    const res = await requestApi<{ data: unknown[] }>({ path: `/v1/changelog/dashboard/${projectId}`, auth: 'session' });
    spinner?.stop();
    if (!res.ok) { log.error(getResponseError(res)); process.exitCode = 1; return; }

    const items = res.data?.data ?? [];
    if (items.length === 0) { if (!options.quiet) log.info('No changelog entries yet.'); return; }
    printOutput(items, { output: options.output ?? 'table', select: options.select, raw: options.raw, defaultColumns: ['id', 'title', 'status', 'published_at', 'created_at'] });
  } catch (err) {
    spinner?.stop();
...
export async function changelogUpdateCommand(id: string, options: ChangelogUpdateOptions = {}): Promise<void> {
  ...
  if (Object.keys(body).length === 0) { log.error('Provide at least one of --title, --content, or --status.'); process.exitCode = 1; return; }

  const spinner = ora('Updating changelog entry...').start();
  try {
    const res = await requestApi({ path: `/v1/changelog/dashboard/${projectId}/${id}`, method: 'PATCH', auth: 'session', body });
    spinner.stop();
    if (!res.ok) { log.error(getResponseError(res)); process.exitCode = 1; return; }
...
export async function changelogDeleteCommand(id: string, options: { project?: string } = {}): Promise<void> {
  ...
  try {
    const res = await requestApi({ path: `/v1/changelog/dashboard/${projectId}/${id}`, method: 'DELETE', auth: 'session' });
    spinner.stop();
    if (!res.ok) { log.error(getResponseError(res)); process.exitCode = 1; return; }

...
export async function feedbackListCommand(options: FeedbackListOptions = {}): Promise<void> {
  ...
  try {
    const res = await requestApi<{ data: unknown[] }>({
      path: `/v1/feedback/inbox/${projectId}`,
      auth: 'session',
    });
    spinner?.stop();
    if (!res.ok) {
    ...
    if (items.length === 0) {
      if (!options.quiet) log.info('No feedback yet.');
      return;
    }
    printOutput(items, {
      ...
      raw: options.raw,
      defaultColumns: ['id', 'type', 'message', 'status', 'created_at'],
    });
  } catch (err) {
    spinner?.stop();
...
export async function feedbackUpdateCommand(
  id: string,
  options: FeedbackUpdateOptions = {}
): Promise<void> {
  const status = options.status;
  if (!status) {
    log.error('--status is required (open, planned, in_progress, done, closed).');
    process.exitCode = 1;
    return;
  }

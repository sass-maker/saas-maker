...
export async function roadmapListCommand(options: RoadmapListOptions = {}): Promise<void> {
  ...
  try {
    const res = await requestApi<{ data: unknown[] }>({ path: `/v1/roadmap/dashboard/${projectId}`, auth: 'session' });
    spinner?.stop();
    if (!res.ok) { log.error(getResponseError(res)); process.exitCode = 1; return; }

    const items = res.data?.data ?? [];
    if (items.length === 0) { if (!options.quiet) log.info('No roadmap items yet.'); return; }
    printOutput(items, { output: options.output ?? 'table', select: options.select, raw: options.raw, defaultColumns: ['id', 'title', 'status', 'votes', 'created_at'] });
  } catch (err) {
    spinner?.stop();
...
export async function roadmapCreateCommand(options: RoadmapCreateOptions = {}): Promise<void> {
  ...
  try {
    ...
    const description = options.description ?? (await rl.question('Description (optional): ')).trim();
...
export async function roadmapUpdateCommand(id: string, options: RoadmapUpdateOptions = {}): Promise<void> {
  ...
  if (Object.keys(body).length === 0) { log.error('Provide at least one of --title, --description, or --status.'); process.exitCode = 1; return; }

  const spinner = ora('Updating roadmap item...').start();
  try {
    const res = await requestApi({ path: `/v1/roadmap/dashboard/${projectId}/${id}`, method: 'PATCH', auth: 'session', body });
    spinner.stop();
    if (!res.ok) { log.error(getResponseError(res)); process.exitCode = 1; return; }
...
export async function roadmapDeleteCommand(id: string, options: { project?: string } = {}): Promise<void> {
  ...
  try {
    const res = await requestApi({ path: `/v1/roadmap/dashboard/${projectId}/${id}`, method: 'DELETE', auth: 'session' });
    spinner.stop();
    if (!res.ok) { log.error(getResponseError(res)); process.exitCode = 1; return; }

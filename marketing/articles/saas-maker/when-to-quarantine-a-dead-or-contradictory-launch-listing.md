...
slug: "/articles/when-to-quarantine-a-dead-or-contradictory-launch-listing"
target_query: "When to quarantine a dead or contradictory launch listing"
search_intent: "Informational - Engineering and product teams evaluating how to manage conflicting or obsolete data in product launch directories and data pipelines."
meta_title: "When to Quarantine a Dead or Contradictory Launch Listing"
meta_description: "Learn how the quarantine pattern isolates suspect launch listings and conflicting metrics without silently erasing historical data."
---

## Outline

1. **Introduction**
   - The challenge of maintaining a reliable catalog.
2. **The Deletion Anti-Pattern**
   - Why simply deleting dead or conflicting listings destroys essential historical context.
3. **The Quarantine Strategy**
   - Defining the quarantine pattern: preserving the data structure while isolating the record.
4. **Trigger 1: Suspect Source Data**
   - How to handle metrics that originate from unverified providers.
5. **Trigger 2: The Dead Destination**
   - Recognizing and handling destinations that no longer resolve.
6. **A Concrete Example from LaunchDesk**
   - Analyzing a real-world quarantined record, examining its flags, provider status, and claim array.
7. **Architecting the Quarantine Implementation**
   - How to project data safely.
8. **Operational Rules for Quarantined Records**
   - Ensuring that automation, shared skills, and continuous integration tests respect the quarantine boundary.
9. **Internal Link Suggestions**
   - Recommended connections to related internal content.
10. **Practical Next Action**
   - Immediate steps for engineering teams.
11. **Non-Publishable Source Notes**
   - Repository evidence supporting the claims in this article.
...
To further contextualize the quarantine strategy within a broader data architecture, consider exploring these related topics:

*   **Generating Public Projections:** Learn how to deterministically filter owner-local JSON data into safe, public-facing exports.
*   **Bounded Data Skills:** Understand how automated research and data acquisition skills interact with checked-in data sets without mutating private state.
*   **Maintaining Lossless Catalogs:** Explore the principles of structuring JSON catalogs to preserve historical classification and Git boundaries.
*   **Deploy-Guard CI Recognition:** Read about how continuous integration pipelines validate data integrity without running untrusted external requests.
...
*The following section is for internal review only and should not be published in the final rendering.*

*   **Evidence of single owner-local catalog:** Supported by `AGENTS.md`, which states that the system owns the single owner-local portfolio catalog at `catalog/projects.json` and its generated, filtered public exports.
*   **Evidence of LaunchDesk dataset generation:** Supported by `AGENTS.md` and `PROJECT_STATUS.md`. The LaunchDesk catalog section at `/launchdesk` is rendered natively from the generated dataset `apps/showcase/src/data/launchdesk.json`. Its source lives in a sibling checkout, and hand-editing the generated JSON is explicitly forbidden.
*   **Evidence of the quarantine flag and structure:** Supported by raw data in `apps/showcase/src/data/launchdesk.json`. The file contains multiple objects illustrating the quarantine structure, including the `"quarantined": true` boolean flag, the `"flags"` array (e.g., `["Suspect source data"]`), `"route": "needs-research"`, unverified provider strings, and a `"claims"` array detailing the conflicting or suspect metrics.
*   **Evidence of Test Suite rules:** Supported by `tooling/skills/test-quality/SKILL.md`, which explicitly mandates: "Do not quarantine, disable, delete, or rewrite tests during diagnosis." The article correctly positions the quarantine action as a data-level pattern, distinct from test suppression.
*   **Evidence of strict public/private boundary:** Supported by `PROJECT_STATUS.md` under the "One editable portfolio catalog" section, which details how the full table and public export are generated, raw private data stays local and ignored by Git, and public builds use the allowlisted projection only.

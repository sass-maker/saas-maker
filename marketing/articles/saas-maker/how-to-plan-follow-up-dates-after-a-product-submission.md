...
target_query: "product submission follow up plan"
search_intent: "learn how to verify product submission directories and plan corrections"
meta_title: "How to plan follow-up dates after a product submission"
meta_description: "Learn how to establish actionable follow-up and verification plans after submitting a product to directories."
---
...
1.  **Why Tracking Follow-Up Matters:** The danger of phantom metrics and the necessity of verifying live placements.
2.  **When and How to Verify Live Placements:** Setting cadences for immediate, short-term, and medium-term verifications.
3.  **Correcting Stale Product Details:** Identifying discrepancies like name mismatches or URL migrations, and executing a formal correction workflow.
4.  **Analyzing ROI: Manual Curated vs. Automated Submissions:** Why curated launches yield verifiable results while automated blasting often results in zero live placements.
5.  **Practical Next Actions:** Concrete steps to log, verify, automate checks, and reconcile your product submissions.

## Internal-Link Suggestions
*   Link "launch platforms" to our internal guide on executing a `launch-campaign`.
*   Link "central catalog" to our documentation on maintaining the `projects.json` structure for site health.
*   Link "automated bulk submissions" to our breakdown of directory strategies.
...
## Source Notes (Internal Review Only - Do Not Publish)

- **Failed Search/Echoing Query Example:** `tooling/config/directory-submissions/campaigns/authority-2026-09.json` (Evidence section for `thestartupinc-correction`, `paggu-correction`, etc. detailing the `zzqqxnonsense7788` control query and the conclusion that 0 fleet listings are live).
- **Stale Product Details (SaaS Maker Docs / NXDOMAIN):** `tooling/config/directory-submissions/campaigns/authority-2026-09.json` (Under `completedItems` > `catalog-name-reconciliation`, detailing the July submission of "SaaS Maker Docs" and the subsequent NXDOMAIN resolution in Sept 2026).
- **Other Name Mismatches:** `tooling/config/directory-submissions/campaigns/authority-2026-09.json` (Detailing Foundry vs SaaS Maker, Email Manager vs Kinetic, Pace vs HeyPace).
- **Directory Strategy and Constraints:** `tooling/config/directory-submissions/directories.json` (Detailing the "Quality over spam" strategy, manual requirements for Product Hunt/Smol Launch, and the automated/blocked statuses of others).
- **Launch Campaign Concept:** Mentioned as a reference to the `launch-campaign` skill (found in `tooling/skills/launch-campaign/SKILL.md`).

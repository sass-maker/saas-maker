...
export interface TaskWorkflowArtifactRow {
  id: string;
  owner_id: string;
  workflow_id: string;
  task_id: string | null;
...
function hydrateTaskCommentRow(
  ...
  return {
    ...
    async createSession(input) {
      await d1
        ...
        .bind(input.token_hash, input.user_id, input.expires_at)
        .run();
    },
    ...
    async createProject(input) {
      ...
      await d1
        ...
        .bind(input.id, input.name, input.slug, input.api_key, input.owner_id, source, gitUrl)
        .run();
    ...
    async listProjectsByOwner(ownerId, source) {
      ...
      const filter =
        source === 'all'
          ...
          : and(
              eq(schema.projects.owner_id, ownerId),
              eq(schema.projects.source, source || 'dashboard')
            );
    ...
    async setVote(input) {
      ...
      if (!existing) {
        ...
        if (input.vote === 1) {
          await d1
            .prepare(`UPDATE feedback SET upvote_count = upvote_count + 1 WHERE id = ?`)
            .bind(input.feedback_id)
            .run();
        } else {
          await d1
            .prepare(`UPDATE feedback SET downvote_count = downvote_count + 1 WHERE id = ?`)
            .bind(input.feedback_id)
            .run();
        }
    ...
    async removeVote(feedbackId, userId) {
      ...
      if (Number(existing.vote) === 1) {
        await d1
          .prepare(`UPDATE feedback SET upvote_count = MAX(upvote_count - 1, 0) WHERE id = ?`)
          .bind(feedbackId)
          .run();
      } else {
        await d1
          .prepare(`UPDATE feedback SET downvote_count = MAX(downvote_count - 1, 0) WHERE id = ?`)
          .bind(feedbackId)
          .run();
      }
    ...
    async hasUpvoted(feedbackId, userId) {
      const row = await d1
        .prepare(`SELECT 1 FROM feedback_votes WHERE feedback_id = ? AND user_id = ? AND vote = 1`)
        .bind(feedbackId, userId)
        .first();
      return !!row;
    },
    ...
    async createWaitlistEntry(input) {
      ...
      await d1
        ...
        .bind(input.id, input.project_id, input.email, input.name, nextPos)
        .run();
      const row = await d1
    ...
    async getWaitlistCount(projectId) {
      const row = await d1
        .prepare(`SELECT COUNT(*) AS total FROM waitlist_entries WHERE project_id = ?`)
        .bind(projectId)
        .first();
    ...
    async listWaitlistEntries(projectId, page, limit) {
      ...
      const { results } = await d1
        ...
        .all();
      return {
        data: results as unknown as WaitlistEntryRecord[],
    ...
    async listAllTestimonials(projectId: string, page: number, limit: number) {
      const offset = (page - 1) * limit;
      const countRow = await d1
        .prepare(`SELECT COUNT(*) AS total FROM testimonials WHERE project_id = ?`)
        .bind(projectId)
        .first();
      const { results } = await d1
        .prepare(
        ...
        .all();
      return {
        data: results as unknown as TestimonialRecord[],
        total: (countRow?.total as number) || 0,
      };
    },
    ...
    async updateTestimonialStatus(id: string, status: string) {
      await d1.prepare(`UPDATE testimonials SET status = ? WHERE id = ?`).bind(status, id).run();
    ...
    async getCliAuthCode(code: string) {
      const row = await d1
        .prepare(`SELECT * FROM cli_auth_codes WHERE code = ?`)
        .bind(code)
        .first();
      return (
        mapRow<{
    ...
    async createCliToken(token: string, userId: string) {
      await d1
        .prepare(`INSERT INTO cli_tokens (token, user_id) VALUES (?, ?)`)
        .bind(token, userId)
        .run();
    },
    ...
    async getCliTokenUser(token: string) {
      const row = await d1
        .prepare(`SELECT user_id FROM cli_tokens WHERE token = ?`)
        .bind(token)
        .first();
    ...
    async createChangelogEntry(input) {
      ...
      await d1
        ...
        .bind(
          ...
          input.updated_at ?? input.created_at ?? null
        )
        .run();
      const row = await d1
    ...
    async deleteChangelogEntry(id) {
      const { meta } = await d1
        .prepare(`DELETE FROM changelog_entries WHERE id = ?`)
        .bind(id)
        .run();
      return (meta.changes ?? 0) > 0;
    },
    ...
    async hasChangelogEntryForTask(taskId: string): Promise<boolean> {
      const row = await d1
        .prepare(`SELECT id FROM changelog_entries WHERE task_id = ? LIMIT 1`)
        .bind(taskId)
        .first();
      return row !== null;
    },
    ...
    async listChangelogEntries(projectId, page, limit) {
      const offset = (page - 1) * limit;
      const countRow = await d1
        .prepare(`SELECT COUNT(*) AS total FROM changelog_entries WHERE project_id = ?`)
        .bind(projectId)
        .first();
      const { results } = await d1
        .prepare(
        ...
        .all();
      return {
        data: results as unknown as ChangelogEntryRecord[],
        total: (countRow?.total as number) || 0,
      };
    },
    ...
    async getProjectAIConfig(
      projectId: string
    ): Promise<{ ai_base_url: string | null; ai_api_key: string | null; ai_model: string | null }> {
      const row = await d1
        .prepare(`SELECT ai_base_url, ai_api_key, ai_model FROM projects WHERE id = ?`)
        .bind(projectId)
        .first();
      if (!row) throw new Error('Project not found');
      return {
        ai_base_url: row.ai_base_url as string | null,
        ai_api_key: row.ai_api_key as string | null,
    ...
    async updateProjectAIConfig(
      projectId: string,
      config: { ai_base_url: string; ai_api_key?: string; ai_model: string }
    ): Promise<void> {
      const existing = await d1
        .prepare(`SELECT ai_api_key FROM projects WHERE id = ?`)
        .bind(projectId)
        .first();
      if (!existing) throw new Error('Project not found');
      const apiKey = config.ai_api_key ?? existing.ai_api_key;
      await d1
        .prepare(`UPDATE projects SET ai_base_url = ?, ai_api_key = ?, ai_model = ? WHERE id = ?`)
        .bind(config.ai_base_url, apiKey, config.ai_model, projectId)
        .run();
    },
    ...
    async getAIUsageStats(
      ...
      return {
        ...
        avg_latency_ms:
          row?.avg_latency_ms != null ? Math.round(row.avg_latency_ms as number) : null,
    ...
    async listAIRequests(
      ...
      limit: number = 50,
      offset: number = 0
    ): Promise<{ data: AIRequestRecord[]; total: number }> {
      const countRow = await d1
        .prepare(`SELECT COUNT(*) AS total FROM ai_requests WHERE project_id = ?`)
        .bind(projectId)
        .first();
      const { results } = await d1
        .prepare(
        ...
        .all();
      return {
        data: results as unknown as AIRequestRecord[],
        total: (countRow?.total as number) || 0,
      };
    },
    ...
    async setRoadmapVote(input) {
      await d1
        ...
        .bind(input.id, input.roadmap_item_id, input.user_identifier, input.vote, input.vote)
        .run();
      const counts = await d1
        ...
        .bind(input.roadmap_item_id)
        .first();
      const castCounts = counts as unknown as { up: number; down: number };
      await d1
        .prepare(`UPDATE roadmap_items SET upvote_count = ?, downvote_count = ? WHERE id = ?`)
        .bind(castCounts.up, castCounts.down, input.roadmap_item_id)
        .run();
    },

    async removeRoadmapVote(roadmapItemId, userIdentifier) {
      const { meta } = await d1
        .prepare(`DELETE FROM roadmap_votes WHERE roadmap_item_id = ? AND user_identifier = ?`)
        .bind(roadmapItemId, userIdentifier)
        .run();
      if ((meta.changes ?? 0) > 0) {
        const counts = await d1
          .prepare(
        ...
        await d1
          .prepare(`UPDATE roadmap_items SET upvote_count = ?, downvote_count = ? WHERE id = ?`)
          .bind(castCounts.up, castCounts.down, roadmapItemId)
          .run();
      }
      return (meta.changes ?? 0) > 0;
    },
    ...
    async getRoadmapVote(roadmapItemId, userIdentifier) {
      const row = await d1
        .prepare(`SELECT vote FROM roadmap_votes WHERE roadmap_item_id = ? AND user_identifier = ?`)
        .bind(roadmapItemId, userIdentifier)
        .first();
      if (!row) return null;
    ...
    async upsertAIMentionConfig(input) {
      await d1
        ...
        .bind(
          ...
          input.perplexity_api_key
        )
        .run();
      const row = await d1
        .prepare(`SELECT * FROM ai_mention_configs WHERE project_id = ?`)
        .bind(input.project_id)
        .first();
    ...
    async deleteAIMentionConfig(projectId) {
      const { meta } = await d1
        .prepare(`DELETE FROM ai_mention_configs WHERE project_id = ?`)
        .bind(projectId)
        .run();
      return (meta.changes ?? 0) > 0;
    },
    ...
    async createAIMentionPrompt(input) {
      await d1
        ...
        .bind(input.id, input.project_id, input.prompt_text, input.category)
        .run();
      const row = await d1
        .prepare(`SELECT * FROM ai_mention_prompts WHERE id = ?`)
        .bind(input.id)
        .first();
    ...
    async listAIMentionPrompts(projectId) {
      const { results } = await d1
        .prepare(`SELECT * FROM ai_mention_prompts WHERE project_id = ? ORDER BY created_at ASC`)
        .bind(projectId)
        .all();
    ...
    async deleteAIMentionPrompt(id) {
      const { meta } = await d1
        .prepare(`DELETE FROM ai_mention_prompts WHERE id = ?`)
        .bind(id)
        .run();
      return (meta.changes ?? 0) > 0;
    },
    ...
    async countAIMentionPrompts(projectId) {
      const row = await d1
        .prepare(`SELECT COUNT(*) AS total FROM ai_mention_prompts WHERE project_id = ?`)
        .bind(projectId)
        .first();
      return (row?.total as number) || 0;
    },
    ...
    async createAIMentionCheck(input) {
      await d1
        .prepare(`INSERT INTO ai_mention_checks (id, project_id, total_queries) VALUES (?, ?, ?)`)
        .bind(input.id, input.project_id, input.total_queries)
        .run();
      const row = await d1
        .prepare(`SELECT * FROM ai_mention_checks WHERE id = ?`)
        .bind(input.id)
        .first();
    ...
    async updateAIMentionCheck(id, input) {
      ...
      if (input.completed_queries !== undefined) {
        sets.push('completed_queries = ?');
        values.push(input.completed_queries);
      }
      if (input.brand_mention_rate !== undefined) {
        sets.push('brand_mention_rate = ?');
        values.push(input.brand_mention_rate);
      }
    ...
    async createAIMentionResult(input) {
      await d1
        ...
        .bind(
          ...
          input.model,
          input.response_text,
          input.brand_mentioned ? 1 : 0,
          input.brand_sentiment,
          input.brand_position,
          input.competitors_mentioned,
          input.citations,
          input.brand_cited ? 1 : 0,
          input.latency_ms
        )
        .run();
    },
    ...
    async listAIMentionResults(checkId) {
      const { results } = await d1
        .prepare(`SELECT * FROM ai_mention_results WHERE check_id = ? ORDER BY created_at ASC`)
        .bind(checkId)
        .all();
    ...
    async getStandards(ownerId: string, type: string): Promise<StandardsRow | null> {
      const row = await d1
        .prepare(`SELECT * FROM standards WHERE owner_id = ? AND type = ?`)
        .bind(ownerId, type)
        .first();
    ...
    async upsertStandards(
      ...
      await d1
        ...
        .bind(
          ...
          ownerId,
          type,
          JSON.stringify(eslintRules),
          JSON.stringify(tsconfigOptions),
    ...
    async getAllStandardsByOwner(ownerId: string): Promise<StandardsRow[]> {
      const { results } = await d1
        .prepare(`SELECT * FROM standards WHERE owner_id = ? ORDER BY type ASC`)
        .bind(ownerId)
        .all();
    ...
    // --- Fleet Metadata ---
    async upsertFleetMetadata(
      ownerId: string,
      project: Omit<FleetMetadataRow, 'id' | 'owner_id'>
    ): Promise<void> {
      await d1
        .prepare(`
      ...
      `)
        .bind(
          ...
          project.name,
          project.framework,
          project.framework_version ?? null,
          project.db,
          project.auth,
          project.deploy,
          project.test_frameworks,
          project.saasmaker_count,
          project.foundry_linked ? 1 : 0
        )
        .run();
    },
    ...
    async getFleetMetadata(ownerId: string): Promise<FleetMetadataRow[]> {
      const results = await d1
        .prepare(`SELECT * FROM fleet_metadata WHERE owner_id = ? ORDER BY name ASC`)
        .bind(ownerId)
        .all();
    ...
    async deleteFleetMetadataBySlugs(ownerId: string, slugs: string[]): Promise<number> {
      ...
      const result = await d1
        .prepare(`DELETE FROM fleet_metadata WHERE owner_id = ? AND slug IN (${placeholders})`)
        .bind(ownerId, ...uniqueSlugs)
        .run();
    ...
    async deleteFleetMetadataExcept(ownerId: string, slugs: string[]): Promise<number> {
      ...
      if (uniqueSlugs.length === 0) {
        const result = await d1
          .prepare(`DELETE FROM fleet_metadata WHERE owner_id = ?`)
          .bind(ownerId)
          .run();
        return result.meta.changes ?? 0;
      }
      ...
      const placeholders = uniqueSlugs.map(() => '?').join(', ');
      const result = await d1
        .prepare(`DELETE FROM fleet_metadata WHERE owner_id = ? AND slug NOT IN (${placeholders})`)
        .bind(ownerId, ...uniqueSlugs)
        .run();
      return result.meta.changes ?? 0;
    },
    ...
    async listTasks(ownerId: string, status?: string, projectSlug?: string): Promise<TaskRow[]> {
      ...
      const { results } = await d1
        ...
        .all();
      return (results ?? []).map(
        (row) => hydrateTaskRow(row as Record<string, unknown>) as TaskRow
      );
    },
    ...
    async updateTask(
      ...
      if (input.pr_status !== undefined) {
        sets.push('pr_status = ?');
        values.push(input.pr_status);
      }
      if (input.commit_sha !== undefined) {
        sets.push('commit_sha = ?');
        values.push(input.commit_sha);
      }
      if (input.deployment_url !== undefined) {
        sets.push('deployment_url = ?');
        values.push(input.deployment_url);
      }
      if (input.deployment_status !== undefined) {
        sets.push('deployment_status = ?');
        values.push(input.deployment_status);
      }
      if (input.blocked_on_user !== undefined) {
        sets.push('blocked_on_user = ?');
        values.push(input.blocked_on_user ? 1 : 0);
      }
    ...
    async claimNextTask(
      ownerId: string,
      input: { worker: string; capability?: string | null; leaseSeconds?: number }
    ): Promise<TaskRow | null> {
      ...
      const { results } = await d1
        ...
        .bind(input.worker, leaseMod, ownerId, capability, capability)
        .all();
    ...
    async failTask(
      id: string,
      ownerId: string,
      input: { worker: string; error?: string | null; maxAttempts?: number }
    ): Promise<{ dead_letter: boolean; attempts: number; requeued: boolean } | null> {
      ...
      const { results } = await d1
        ...
        .bind(maxAttempts, maxAttempts, input.error ?? null, id, ownerId, input.worker)
        .all();
    ...
    async heartbeatTask(
      ...
      const { meta } = await d1
        ...
        .bind(leaseMod, id, ownerId, worker)
        .run();
      return (meta.changes ?? 0) > 0;
    },
    ...
    async listTaskComments(ownerId: string, taskId: string): Promise<TaskCommentRow[]> {
      const { results } = await d1
        ...
        .all();
      return (results ?? []).map(
        (row) => hydrateTaskCommentRow(row as Record<string, unknown>) as TaskCommentRow
      );
    },
    ...
    async createTaskComment(
      ...
      await d1
        ...
        .bind(id, ownerId, taskId, authorType, input.body, resolvesBlocker, marksDone)
        .run();
      if (resolvesBlocker) {
        await d1
          .prepare(
      ...
      if (input.sync_to_description) {
        ...
        await d1
          ...
          .bind(descriptionNote, descriptionNote, taskId, ownerId)
          .run();
      }
    ...
    async createTaskWorkflow(
    ...
    ): Promise<TaskWorkflowRow | null> {
      const taskId = input.task_id ?? null;
      let projectSlug = input.project_slug ?? null;
      if (taskId) {
        const task = await d1
          .prepare(`SELECT id, project_slug FROM tasks WHERE id = ? AND owner_id = ?`)
          .bind(taskId, ownerId)
          .first<{ id: string; project_slug: string | null }>();
        if (!task) return null;
      ...
      await d1
        ...
        .run();
      const row = await d1.prepare(`SELECT * FROM task_workflows WHERE id = ?`).bind(id).first();
      return row as unknown as TaskWorkflowRow;
    },

    async listTaskWorkflows(
      ownerId: string,
      input: { task_id?: string; project_slug?: string; status?: string; limit?: number } = {}
    ): Promise<TaskWorkflowRow[]> {
      ...
      const { results } = await d1
        .prepare(
          `SELECT * FROM task_workflows WHERE ${conditions.join(' AND ')} ORDER BY updated_at DESC, created_at DESC LIMIT ?`
        )
        .bind(...values)
        .all();
      return results as unknown as TaskWorkflowRow[];
    },

    async getTaskWorkflow(id: string, ownerId: string): Promise<TaskWorkflowRow | null> {
      const row = await d1
        .prepare(`SELECT * FROM task_workflows WHERE id = ? AND owner_id = ?`)
        .bind(id, ownerId)
        .first();
      return row as unknown as TaskWorkflowRow | null;
    },
    ...
    async updateTaskWorkflow(
      ...
      if (input.task_id !== undefined) {
        if (input.task_id) {
          const task = await d1
            .prepare(`SELECT id, project_slug FROM tasks WHERE id = ? AND owner_id = ?`)
            .bind(input.task_id, ownerId)
            .first<{ id: string; project_slug: string | null }>();
          if (!task) return null;
          if (input.project_slug === undefined) input.project_slug = task.project_slug;
        }
        sets.push('task_id = ?');
        values.push(input.task_id);
      }
      ...
      if (input.context_markdown !== undefined) {
        sets.push('context_markdown = ?');
        values.push(input.context_markdown);
      }
      ...
      if (input.last_run_id !== undefined) {
        sets.push('last_run_id = ?');
        values.push(input.last_run_id);
      }
      if (sets.length === 0) {
        const row = await d1
          .prepare(`SELECT * FROM task_workflows WHERE id = ? AND owner_id = ?`)
      ...
      values.push(id, ownerId);
      await d1
        .prepare(`UPDATE task_workflows SET ${sets.join(', ')} WHERE id = ? AND owner_id = ?`)
        .bind(...values)
        .run();
      const row = await d1
        .prepare(`SELECT * FROM task_workflows WHERE id = ? AND owner_id = ?`)
    ...
    async createTaskWorkflowArtifact(
      ownerId: string,
      workflowId: string,
      input: {
        name: string;
    ...
    ): Promise<TaskWorkflowArtifactRow | null> {
      const workflow = await d1
        .prepare(`SELECT * FROM task_workflows WHERE id = ? AND owner_id = ?`)
        .bind(workflowId, ownerId)
        .first<TaskWorkflowRow>();
      ...
      await d1
        ...
        .bind(
          ...
          ownerId,
          workflowId,
          workflow.task_id ?? null,
          workflow.project_slug ?? null,
        ...
        .run();
      const row = await d1
        .prepare(`SELECT * FROM task_workflow_artifacts WHERE id = ?`)
        .bind(id)
        .first();
      return row as unknown as TaskWorkflowArtifactRow;
    },

    async listTaskWorkflowArtifacts(
      ownerId: string,
      workflowId: string
    ): Promise<TaskWorkflowArtifactRow[]> {
      const { results } = await d1
        .prepare(
          `SELECT * FROM task_workflow_artifacts WHERE owner_id = ? AND workflow_id = ? ORDER BY created_at DESC`
        )
        .bind(ownerId, workflowId)
        .all();
      return results as unknown as TaskWorkflowArtifactRow[];
    },

    async getTaskWorkflowArtifactByShareToken(
      shareToken: string
    ): Promise<TaskWorkflowArtifactRow | null> {
      const row = await d1
        .prepare(`SELECT * FROM task_workflow_artifacts WHERE share_token = ?`)
        .bind(shareToken)
        .first();
      return row as unknown as TaskWorkflowArtifactRow | null;
    },

    async createSymphonyAuditEvent(
      ownerId: string,
      input: {
        task_id?: string | null;
        action: string;
      ...
      await d1
        ...
        .run();
      const row = await d1
        .prepare(`SELECT * FROM symphony_audit_log WHERE id = ?`)
        .bind(id)
        .first();
      return row as unknown as SymphonyAuditLogRow;
    },

    async listSymphonyAuditEvents(
      ownerId: string,
    ...
    async createSymphonyRun(
      ...
      await d1
        ...
        .run();
      const row = await d1.prepare(`SELECT * FROM symphony_runs WHERE id = ?`).bind(id).first();
      return row as unknown as SymphonyRunRow;
    },

    async listSymphonyRuns(
      ownerId: string,
      input: { task_id?: string; limit?: number } = {}
    ): Promise<SymphonyRunRow[]> {
      const limit = Math.min(Math.max(input.limit ?? 50, 1), 200);
    ...
    async getSymphonyMemory(ownerId: string): Promise<SymphonyMemoryRow | null> {
      const row = await d1
        .prepare(`SELECT owner_id, content, updated_at FROM symphony_memory WHERE owner_id = ?`)
        .bind(ownerId)
        .first();

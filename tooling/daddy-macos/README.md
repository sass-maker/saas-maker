...
The Swift copy owns the approved series palette, compact button geometry, native menu bar open/quit/status controls, an active-work quit prompt, and opt-in local completion notices. Each app keeps its existing theme and button names as adapters, as well as its own semantic color rules, screens, and behavior. PerformanceDaddy enables its existing hover wash through `hoverFeedback`. The main window in each app is single-instance and can reopen from the menu bar after closing. PerformanceDaddy's existing sampler and BrowserDaddy's consent-gated collector continue while their apps remain open; both offer opt-in launch at login. BrowserDaddy can pause attention tracking without stopping periodic history sync; the watcher closes the current segment and leaves paused time as a gap. StorageDaddy scans and ContextDaddy refreshes remain explicit user actions. StorageDaddy and PerformanceDaddy offer optional notices when a user-started scan or diagnosis completes with the main window hidden. A change to the canonical source must be copied into all four apps and pass the candidate copy check before release; it does not update installed apps by itself.

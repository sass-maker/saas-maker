...
export const FLEET_HEALTH_CONTRACTS = {
  ...
  'fleet-dashboard': {
    ...
    auth: { required: true },
    requiredEnv: { build: [], runtime: [] },
    deployTarget: 'Manual operations host (cutover pending)',
    githubWorkflow: null,
    smokeCommand: null,
  },
  'mobile-dev-cockpit': {
    displayName: 'Mobile Dev Cockpit',
    prodUrl: null,
    expectedStatus: 200,
    criticalRoutes: [],
    auth: { required: false },
    requiredEnv: { build: [], runtime: [] },
    deployTarget: 'Native application',
    githubWorkflow: null,
    smokeCommand: null,
  },
  motion: {
  ...
  'personal-website': {
    ...
    prodUrl: 'https://sarthakagrawal.dev',
    expectedStatus: 200,
    criticalRoutes: ['/'],
    auth: { required: false },
    requiredEnv: { build: [], runtime: [] },
    ...
    smokeCommand: 'pnpm run fleet:prod-smoke --project personal-website',
  },
  aliveville: {
  ...
  'free-ai': {
    ...
    automation: {
      ...
      protectedProbe: {
        label: 'chat-completions-rejects-missing-key',
        method: 'POST',
      ...
      freshness: {
        mode: 'rolling',
        windowSeconds: null,
        note: 'HealthStateDO rolling per-model window; no scheduled recompute to go stale.',
      },
    },
    providerEvidence: {
    ...
    storage: [
      {
        ...
        reconstruction: 'Cloudflare D1 export; analytics/ledger rebuild from live traffic',
        migrationGuard: 'pnpm typecheck + pnpm test',
      },
      {
        binding: 'HEALTH_KV',
        kind: 'KV',
        owner: 'free-ai',
        source: 'HealthStateDO writes',
        reconstruction: 'ephemeral cache; rebuilds from D1 + live provider probes',
        migrationGuard: 'n/a (cache)',
      },
      {
        binding: 'HEALTH_DO',
        kind: 'Durable Object',
        owner: 'free-ai',
        source: 'per-model state machine',
        reconstruction: 'rebuilds from live traffic',
        migrationGuard: 'DO migration tags v1, v2',
      },
      {
        binding: 'RATE_LIMIT_DO',
        kind: 'Durable Object',
        owner: 'free-ai',
        source: 'per-IP token bucket',
        reconstruction: 'ephemeral; rebuilds from live traffic',
        migrationGuard: 'DO migration tag v1',
      },
      {
        binding: 'NEURON_BUDGET',
        kind: 'Durable Object',
        owner: 'free-ai',
        source: 'daily Neuron counter',
        reconstruction: 'resets daily; no backfill needed',
        migrationGuard: 'DO migration tag v2',
      },
    ],
  },
  'high-signal': {
  ...
  'knowledge-base': {
    ...
    automation: {
      ...
      protectedProbe: {
        label: 'indexes-rejects-missing-key',
        method: 'GET',
        path: '/v1/indexes',
        okStatuses: [401],
      },
      freshness: {
        mode: 'on-demand',
        windowSeconds: null,
        note: 'Corpora are opt-in; no scheduled freshness window. A domain in "ingesting" with no recent job progress is stale.',
      },
    },
    providerEvidence: {
      costCap: null,
      ...
      privacy: {
        ...
        source: 'classifyIngestFailure + job/trace/eval routes',
      },
    ...
    jobs: [
      {
        ...
        timeoutMs: 45000,
        retries:
          'queue max_retries 3; poison-input ack after 5 attempts; per-file retry exponential backoff capped at 300s',
        idempotency:
          'deterministic chunk IDs; D1 INSERT OR REPLACE; Vectorize upsert by vector ID; parse artifact keyed by content hash; idempotency_key + idempotent_replay',
        failureState:
          "job status='failed' + last_error + attempts in D1; failure_classification on job detail route",
      ...
      },
      {
        ...
        retries: 'n/a',
        idempotency:
          'isIngestJobLeaseActive skips files with status=running + locked_by != current + locked_at within 5m',
    ...
    storage: [
      {
        ...
        reconstruction:
          'Cloudflare D1 export; metadata reconstructable from R2 raw docs + re-ingest',
        migrationGuard: 'pnpm check + audit:d1-migrations; wrangler d1 migrations apply is manual',
      },
      {
        binding: 'RAW_DOCS',
        kind: 'R2',
        ...
        reconstruction:
          'R2 is the authoritative source for raw documents; re-ingest from R2 rebuilds D1 + Vectorize',
        migrationGuard: 'n/a (source of truth)',
      },
      {
        binding: 'VECTORIZE',
        kind: 'Vectorize',
        owner: 'knowledge-base',
        source: 'derived from R2 + D1',
        reconstruction:
          'reconstructable: re-ingest from R2 via /v1/kb/files/:file_id/reprocess or /v1/kb/schemas/:domain/reprocess',
        migrationGuard: 'audit:vectorize-embedding-bindings + audit:vectorize-metadata-indexes',
      },
      {
        binding: 'VECTORIZE_384/768/1024',
        kind: 'Vectorize',
        owner: 'knowledge-base',
        source: 'derived from R2 + D1',
        reconstruction: 'same reconstruction path',
        migrationGuard: 'same audit gates',
      },
      {
        binding: 'AI',
        kind: 'Workers AI',
        owner: 'knowledge-base',
        source: 'Cloudflare',
        reconstruction: 'n/a (managed service)',
        migrationGuard: 'n/a',
      },
      {
        binding: 'FREE_AI',
        kind: 'Service binding',
        owner: 'knowledge-base (consumer)',
        source: 'free-ai repo',
        reconstruction: 'n/a (delegated to free-ai)',
        migrationGuard: 'audit:free-ai-embedding-contract',
      },
      {
        binding: 'RAG_ANALYTICS',
        kind: 'Analytics Engine',
        owner: 'knowledge-base',
        source: 'per-query events',
        reconstruction: 'ephemeral; not a source of truth',
        migrationGuard: 'n/a',
      },
    ],
  },

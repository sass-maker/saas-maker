...
describe('task-queue worker endpoints', () => {
  it('claims the next task for a capability', async () => {
    mockDb.claimNextTask.mockResolvedValue({
      id: 't1',
      capability: 'review',
      status: 'in_progress',
    });
    ...
    expect(mockDb.claimNextTask).toHaveBeenCalledWith(
      'user-1',
      expect.objectContaining({ worker: 'codevetter@host', capability: 'review' })
    );
  });
  ...
  it('requires a worker id to claim', async () => {
    const res = await request(
      '/v1/tasks/claim',
      {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ capability: 'review' }),
      },
      { DB: {} }
    );
  ...
  it('clamps lease_seconds into range', async () => {
    ...
    expect(mockDb.claimNextTask).toHaveBeenCalledWith(
      'user-1',
      expect.objectContaining({ leaseSeconds: 3600 })
    );
  });
  ...
  it('completes a task held by the worker', async () => {
    ...
    expect(mockDb.createTaskComment).toHaveBeenCalledWith(
      'user-1',
      't1',
      expect.objectContaining({ author_type: 'agent', body: 'looks good' })
    );
  });
  ...
  it('requeues on fail under max attempts, dead-letters past it', async () => {
    ...
    expect(((await res.json()) as { outcome: { dead_letter: boolean } }).outcome.dead_letter).toBe(
      true
    );
  });

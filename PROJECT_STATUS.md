...
- **2026-06-20 — Knowledge/RAG removal:** Deleted `/v1/knowledge/*`, Cockpit knowledge UI, SDK `KnowledgeService`, API RAG bindings, and D1 knowledge tables (`0021_drop_knowledge.sql`). SaaS Maker is not a search/RAG hub. Deleted `packages/tooling/*` (eslint-config, prettier-config, tsconfig, astro-landing, eslint-plugin-fallow), `packages/blocks/ops`, and shelved `packages/blocks/views/`. Cockpit astro overlay script is local under `apps/cockpit/scripts/`.

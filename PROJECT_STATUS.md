...
- **2026-07-20 — Foundry helper source resync:** Synchronized Mobile Dev Cockpit, Drank, Reel Pipeline, Fleet Ops, and Fleet-owned PSI Swarm from their latest clean pushed `main` revisions into their canonical monorepo paths. Removed the obsolete OpenShorts gitlink from the monorepo root while preserving the two active Reel Pipeline engine pins. This updates source ownership only; production deploy identities and old-repository archival remain behind the existing cutover gates.

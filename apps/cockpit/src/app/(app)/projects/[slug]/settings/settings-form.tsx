...
import { Save, Trash2, MessageSquare, Users, Star, Megaphone } from 'lucide-react';
...
const features = [
  {
    ...
    label: 'Feedback',
    icon: MessageSquare,
    pkg: '@saas-maker/feedback-widget',
  },
  { key: 'waitlist' as const, label: 'Waitlist', icon: Users, pkg: '@saas-maker/waitlist-widget' },
  {
    ...
    label: 'Testimonials',
    icon: Star,
    pkg: '@saas-maker/testimonials-widget',
  },
  {
    key: 'changelog' as const,
    label: 'Changelog',
    icon: Megaphone,
    pkg: '@saas-maker/changelog-widget',
  },
];
...
export function SettingsForm({ project, featureCounts }: SettingsFormProps) {
  ...
  const hasChanges = name !== project.name || notes !== (project.readme ?? '');

  async function handleDelete() {
    setDeleting(true);
    try {
      const token = await getToken();
      await apiFetch(`/v1/projects/${project.id}`, { method: 'DELETE' }, token);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Use this key to authenticate your feedback widget.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
      ...
      {featureCounts && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Services enabled for this project.</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {features.map((f) => {
                ...
                return (
                  <div key={f.key} className="flex items-center gap-3 rounded-lg border p-3">
                    <f.icon
                      className={`h-5 w-5 shrink-0 ${active ? 'text-foreground' : 'text-muted-foreground/40'}`}
                    />
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium ${active ? '' : 'text-muted-foreground'}`}>
                        {f.label}
                      </p>
                      {f.pkg && <p className="text-xs text-muted-foreground truncate">{f.pkg}</p>}
                    </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Install the feedback widget in your application.</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              remove the project, all feedback data, and API keys. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>

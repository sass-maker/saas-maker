...
export default async function WorkflowArtifactPage({ params }: { params: Promise<{ shareToken: string }> }) {
    ...
    <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6 lg:px-8">
      <article className="mx-auto max-w-3xl">
        <div className="mb-6 space-y-2 border-b pb-4">
          <p className="text-xs uppercase tracking-wide text-muted-foreground">Task workflow artifact</p>
          ...
          <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
            ...
            <span>{new Date(artifact.created_at).toLocaleString('en-US', { timeZone: 'UTC' })}</span>
          </div>
        </div>

...
export async function POST(req: Request) {
  ...
  if (!isLocalAuthBypassEnabled(requestHeaders.get('host'))) {
    return NextResponse.json(
      { error: 'Task execution is only available locally' },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (runTasks.some((item) => item.status === 'done' || isBlockedTask(item))) {
      return NextResponse.json(
        { error: 'Blocked and done tasks cannot be dispatched' },
        { status: 400 }
      );
    }

    const agentUsage = readAgentUsage();
    const runs = buildSymphonyBatchRuns(runTasks, {
      agent,
      agentCommand,
      memory,
    ...
    return NextResponse.json(
      {
        error: 'Failed to start task agent',

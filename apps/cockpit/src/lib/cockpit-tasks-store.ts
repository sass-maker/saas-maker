...
export async function ensureCockpitUser(user: CockpitUserInput, db = getCockpitD1()) {
  ...
  await db
    .prepare(`INSERT INTO users (id, email, name, avatar_url)
    VALUES (?, ?, ?, ?)
    ...
    .run();
  const row =
    (await db.prepare('SELECT id FROM users WHERE id = ?').bind(user.id).first<{ id: string }>()) ??
...
export async function getDefaultCockpitOwnerId(db = getCockpitD1()) {
  const taskOwner = await db
    .prepare('SELECT owner_id FROM tasks GROUP BY owner_id ORDER BY COUNT(*) DESC LIMIT 1')
...
function normalizeTaskPatch(input: TaskPatch) {
  ...
  const blockedOnUser =
    typeof input.blocked_on_user === 'boolean' ? input.blocked_on_user : undefined;
  return {
...
export async function createCockpitTask(
  ownerId: string,
...
export async function listCockpitTaskComments(taskId: string, db = getCockpitD1()) {
  const { results } = await db
    ...
    .all<Record<string, unknown>>();
  return (results ?? [])
    .map((row) => hydrateComment(row))
    .filter((comment): comment is TaskCommentRow => Boolean(comment));
}

export async function createCockpitTaskComment(
  taskId: string,
  ...
  await db
    .prepare(`INSERT INTO task_comments (id, owner_id, task_id, author_type, body, resolves_blocker, marks_done)
    VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .bind(
      id,
      task.owner_id,
  ...
  if (resolvesBlocker) {
    await db
      .prepare("UPDATE tasks SET blocked_on_user = 0, updated_at = datetime('now') WHERE id = ?")
      .bind(taskId)
      .run();
  }
  if (marksDone) {
    await db
      .prepare(
        "UPDATE tasks SET status = 'done', blocked_on_user = 0, updated_at = datetime('now') WHERE id = ?"
      )
      .bind(taskId)
      .run();
  }
...
export async function listCockpitRuns(limit = 200, taskId?: string, db = getCockpitD1()) {
  ...
  if (taskId) {
    const { results } = await db
      .prepare(
        'SELECT * FROM symphony_runs WHERE task_id = ? ORDER BY started_at DESC, created_at DESC LIMIT ?'
      )
  ...
  const { results } = await db
    .prepare('SELECT * FROM symphony_runs ORDER BY started_at DESC, created_at DESC LIMIT ?')
    .bind(safeLimit)
    .all<SymphonyRunRow>();
  return results ?? [];
}
...
export async function listCockpitProjectSlugs(db = getCockpitD1()) {
  const { results } = await db
    .prepare('SELECT slug FROM fleet_metadata ORDER BY slug ASC')
    .all<{ slug?: string }>();
...
export async function getCockpitSymphonyMemory(ownerId: string, db = getCockpitD1()) {
  ...
  const latest = await db
    .prepare(
      'SELECT owner_id, content, updated_at FROM symphony_memory ORDER BY updated_at DESC LIMIT 1'
    )
    .first<{ content?: string }>();
  return latest?.content ?? '';
}

export async function updateCockpitSymphonyMemory(
  ownerId: string,
  content: string,
  db = getCockpitD1()
) {
  await db
    .prepare(`INSERT INTO symphony_memory (owner_id, content, updated_at)
    VALUES (?, ?, datetime('now'))
  ...
  return db
    .prepare('SELECT owner_id, content, updated_at FROM symphony_memory WHERE owner_id = ?')
    .bind(ownerId)
    .first<{ owner_id: string; content: string; updated_at: string }>();
}

export async function createCockpitTaskWorkflow(
  ownerId: string,
  ...
  const id = crypto.randomUUID();
  await db
    .prepare(`INSERT INTO task_workflows (
...
export async function listCockpitTaskWorkflows(
  input: {
    task_id?: string;
    project_slug?: string;
    status?: TaskWorkflowRow['status'];
    limit?: number;
  } = {},
  db = getCockpitD1()
) {
  ...
  const { results } = await db
    .prepare(
      `SELECT * FROM task_workflows ${where} ORDER BY updated_at DESC, created_at DESC LIMIT ?`
    )
    .bind(...values)
    .all<TaskWorkflowRow>();
  return results ?? [];
...
export async function updateCockpitTaskWorkflow(
  id: string,
  input: Partial<TaskWorkflowInput & { last_run_id: string | null }>,
  db = getCockpitD1()
) {
  const patch = { ...input };
  if (patch.task_id) {
  ...
  values.push(id);
  await db
    .prepare(`UPDATE task_workflows SET ${sets.join(', ')} WHERE id = ?`)
    .bind(...values)
    .run();
  return getCockpitTaskWorkflow(id, db);
}

export async function createCockpitTaskWorkflowArtifact(
  workflowId: string,
  input: { name: string; content_markdown: string; run_id?: string | null },
  db = getCockpitD1()
) {
  ...
  await db
    .prepare(`INSERT INTO task_workflow_artifacts (
  ...
  return db
    ...
    .bind(id)
    .first<TaskWorkflowArtifactRow>();
}

export async function listCockpitTaskWorkflowArtifacts(workflowId: string, db = getCockpitD1()) {
  const { results } = await db
    .prepare('SELECT * FROM task_workflow_artifacts WHERE workflow_id = ? ORDER BY created_at DESC')
    .bind(workflowId)
    .all<TaskWorkflowArtifactRow>();
  return results ?? [];
}

export async function getCockpitTaskWorkflowArtifactByShareToken(
  shareToken: string,
  db = getCockpitD1()
) {
  return db
